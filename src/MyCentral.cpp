...
PVariable MyCentral::getServiceMessages(PRpcClientInfo clientInfo, bool returnId, const std::string &language, bool checkAcls) {
  try {
    auto serviceMessages = ICentral::getServiceMessages(clientInfo, returnId, language, checkAcls);
    ...
    for (auto &interface : interfaces) {
      ...
      for (auto &element : ccuServiceMessages) {
        ...
        if (returnId) {
          ...
          newElement->structValue->emplace("CHANNEL", std::make_shared<Variable>(0));
